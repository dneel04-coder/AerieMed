...
Future<PdfDocument> pdfDocumentFromUri(
  ...
  try {
    ...
    // Backport of upstream fix (pdfrx_engine v0.4.6): `cache!.blockSize` on
    // one line doesn't promote `cache` to non-null for later lines inside
    // this async closure, since it's a captured outer parameter, not a
    // local `cache` promotion doesn't survive across the `await` calls
    // below. Capturing a non-null local before the closure fixes it.
